    ...
    <div className="page-container">
      <header className="main-header">
        ...
        <div className="header-actions">
          ...
          <button className="action-btn-icon glass" title="Add Manual Deal" onClick={() => setShowAddModal(true)}>
            <Plus size={18} />
          </button>
        </div>
      </header>
      ...
      <ConfirmModal 
        ...
        message={`Are you sure you want to delete the deal for "${deletingDeal?.company_name}"? Archiving will instantly delete the pipeline record.`}
      ...
      <style>{`
        ...
        .deals-list { display: flex; flex-direction: column; gap: 1.5rem; }
...
const DealCard = ({ deal, isExpanded, onToggle, refresh, onDelete }) => {
  ...
  return (
    <div className={`deal-card glass animate-fade-in ${getSeverity(deal.intent_score)} ${isExpanded ? 'expanded' : ''}`}>
      <div className="deal-main" onClick={onToggle}>
        <div className="deal-info">
          <div className="deal-brand">
            <div className="deal-icon glass"><Zap size={20} /></div>
            <div>
              <h3 className="outfit">{deal.company_name}</h3>
              <div className="deal-comms"><Mail size={14} /><span>{deal.emails?.length || 0} Communications</span></div>
            </div>
          </div>

          <div className="urgency-meter">
            <div className="meter-header"><TrendingUp size={14} /><span>Pipeline Score</span></div>
            <div className="score-display">
               <span className="meter-value outfit">{Math.round(deal.intent_score)}%</span>
               <div className="urgency-tag">{deal.intent_score >= 80 ? 'Critical' : deal.intent_score >= 50 ? 'Urgent' : 'Steady'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="deal-actions">
          <div className="action-stack">
            {deal.intent_score >= 80 && <div className="hot-pulse-outer"><div className="hot-pulse-inner"></div></div>}
            <a href="https://mail.google.com/" target="_blank" rel="noopener noreferrer" className="btn-icon mini" onClick={(e) => e.stopPropagation()} title="Open Gmail"><Mail size={18} /></a>
            <button className="btn-icon mini delete-btn" onClick={(e) => { e.stopPropagation(); onDelete(); }} title="Delete Deal & Archive"><Trash2 size={16} /></button>
            <button className="btn-icon mini">{isExpanded ? <ChevronUp size={20} /> : <MessageSquare size={18} />}</button>
          </div>
        </div>
      </div>
        ...
        <div className="deal-details animate-fade-in">
          <div className="details-grid">
            <div className="logs-panel">
              <LogManager type="deal" parentId={deal._id} logs={deal.logs || []} onUpdate={refresh} />
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        .deal-card { border-left: 6px solid transparent; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); overflow: hidden; }
        ...
        .deal-main { padding: 2rem; display: flex; justify-content: space-between; align-items: center; cursor: pointer; }
        .deal-info { display: flex; align-items: center; gap: 6rem; flex: 1; }
        .deal-brand { display: flex; align-items: center; gap: 1.5rem; min-width: 280px; }
        .deal-icon { width: 54px; height: 54px; display: flex; align-items: center; justify-content: center; color: var(--accent-primary); border-radius: 14px; background: rgba(255, 255, 255, 0.03); }
        .deal-brand h3 { font-size: 1.75rem; font-weight: 800; text-transform: capitalize; margin-bottom: 0.3rem; }
        .deal-comms { display: flex; align-items: center; gap: 0.6rem; font-size: 0.85rem; color: var(--text-muted); font-weight: 700; }
        .urgency-meter { display: flex; flex-direction: column; gap: 0.4rem; }
        .meter-header { display: flex; align-items: center; gap: 0.6rem; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; color: var(--text-muted); letter-spacing: 0.1em; }
        .score-display { display: flex; align-items: center; gap: 1.25rem; }
        .meter-value { font-size: 2rem; font-weight: 900; color: var(--text-main); }
        .urgency-tag { font-size: 0.75rem; letter-spacing: 0.05em; text-transform: uppercase; font-weight: 800; padding: 0.25rem 0.6rem; border-radius: 4px; background: rgba(255, 255, 255, 0.05); color: var(--text-muted); }
        .hot-pulse-outer { position: relative; width: 14px; height: 14px; }
        .hot-pulse-inner { width: 100%; height: 100%; border-radius: 50%; background: var(--accent-danger); box-shadow: 0 0 12px var(--accent-danger); animation: pulse-ring 2s infinite ease-out; }
        ...
        .action-stack { display: flex; align-items: center; gap: 1rem; }
        ...
        .deal-details { padding: 0 2rem 3rem 2rem; border-top: 1px solid var(--glass-border); background: rgba(0, 0, 0, 0.15); }
      `}</style>
    </div>
  );
};
